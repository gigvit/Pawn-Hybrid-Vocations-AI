-- Purpose:
-- Capture a deeper profile of current main_pawn MainDecisions from DecisionEvaluationModule.
-- Run once with main_pawn Job01 and once with main_pawn Job07, then compare the two JSON files.
-- Output:
--   reframework/data/ce_dump/main_pawn_main_decision_profile_job<job>_<timestamp>.json
...
local function build_entry(item, index, item_source)
    local named_fields = build_named_field_map(item, INTERESTING_FIELDS)

    return {
        index = index,
        item = serialize_object(item, item_source),
        named_fields = named_fields,
        scalar_profile = build_profile_key(named_fields, PROFILE_FIELDS),
        enumish_profile = build_profile_key(named_fields, ENUMISH_PROFILE_FIELDS),
        combined_profile = build_profile_key(named_fields, INTERESTING_FIELDS),
...
local enumish_profile_counts = {}
local combined_profile_counts = {}
...
if main_decision_count ~= nil then
    ...
    for index = 0, max_items - 1 do
        ...
        increment_counter(enumish_profile_counts, entry.enumish_profile)
        increment_counter(combined_profile_counts, entry.combined_profile)
    end
end
...
local output = {
    tag = "main_pawn_main_decision_profile_screen",
    generated_at = os.date("%Y-%m-%d %H:%M:%S"),
    output_note = "Capture a deeper profile of current main_pawn MainDecisions. Run once in Job01 and once in Job07, then compare the JSON files.",
    actor = {
    ...
    main_decisions = {
        ...
        enumish_profile_counts = enumish_profile_counts,
        combined_profile_counts = combined_profile_counts,
        entries = entries,
    },
}

local output_path = "ce_dump/main_pawn_main_decision_profile_" .. job_suffix .. "_" .. os.date("%Y%m%d_%H%M%S") .. ".json"
json.dump_file(output_path, output)
print("[main_pawn_main_decision_profile] wrote " .. output_path)
return output_path
