...
Версия мода: `2.0.0-alpha24-preservation`
...
## Правило Сохранения Знаний

Архив знаний считается частью результата, а не мусором после консолидации. Его нельзя удалять ради уменьшения количества файлов, потому что именно там лежат причины принятых решений, отвергнутые гипотезы, следы live-тестов, CE/DevTools выводы и разбор Bestiary.

Разрешённая чистка:

- переносить повторяющиеся выводы в короткие top-level документы;
- добавлять индекс или пометки `historical`, если сведения устарели;
- исправлять ссылки на текущие entry points, если это не искажает историю.

Запрещённая чистка:

- удалять `docs/archive/KNOWLEDGE_BASE_RU.md`, `docs/archive/KNOWLEDGE_BASE_EN.md` или `docs/archive/KNOWLEDGE_BASE_LEGACY.md` без отдельного ручного review;
- переписывать исторические выводы так, будто они были текущими инструкциями;
- выкидывать неудачные попытки, если они объясняют, почему мы больше не идём этим путём.
