...
This file is the standalone security policy for this repository.
It complements `E:\[Codex]\rules.md`, which defines the broader documentation rules.

Этот файл содержит отдельную security-политику репозитория.
Он дополняет `E:\[Codex]\rules.md`, где описаны общие правила документации.
...
The main branch is intentionally designed to avoid online, pawn-share, and rental hot paths by default.
...
Examples of sensitive engine paths for this project:
...
- or contact the maintainer privately at `gig.for.googl3@gmail.com`
...
- avoid network or share hooks in the hot path unless there is a very specific reason
...
Основная ветка по умолчанию намеренно устроена так, чтобы не трогать online-, pawn-share- и rental-hot path.
...
Примеры чувствительных путей движка для этого проекта:
...
- или написать maintainer приватно на `gig.for.googl3@gmail.com`
...
- не использовать network- или share-hooks в hot path без очень конкретной причины
